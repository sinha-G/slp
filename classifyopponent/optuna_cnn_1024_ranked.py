...
def objective(trial, dataloaders, study_name):
    ...
    def evaluate_test(model, dataloader, criterion, study_name):
        ...
        confusion_matrix_path = 'data\\classifyopponent\\confusion_matrices\\' + study_name + '\\'
        ...
        with torch.no_grad():  # No gradients needed
            ...
            sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', xticklabels=opponents, yticklabels=opponents)
...
def main():
    ...
    study_name = current_datetime_string + "Basic CNN - Classify Fox's Opponent in Top 5"

    # Set up logging file
    log_file = 'data\\classifyopponent\\logs\\' + study_name + ' Log.txt'
    logging.basicConfig(filename=log_file, level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

    save_path = 'C:/Users/jaspa/Grant ML/slp/data'
    file_paths, labels = load_data(save_path)
    loaders = prepare_data_loaders(file_paths, labels, batch_size = 128)
