...
class CustomNet(nn.Module):
    def __init__(self, trial,num_classes):
        ...
        dropout_rate_conv = trial.suggest_float("dropout_rate", 0, 0.35)
        dropout1d_rate_conv = trial.suggest_float("dropout_rate", 0, 0.7)
        dropout_rate_fc = trial.suggest_float("dropout_rate", 0, 0.35)
        dropout_rate_conv /= 2
        ...
        for i in range(2, num_conv_layers):
            kernel_size_i = trial.suggest_int(f"kernel_size_{i}", 3, 11)
        ...
        conv_layers.append(nn.Conv1d(   
            in_channels, out_channels_middle, kernel_size_last,
